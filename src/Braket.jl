...
Currently, only `:OpenQASM` (the OpenQASM3 representation) is supported.

By default, `IRType` is initialized to `:OpenQASM`. The current default value
can be checked by calling `IRType[]`.
