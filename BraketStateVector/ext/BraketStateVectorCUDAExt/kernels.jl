...
function apply_observable_kernel!(
    sv::CuDeviceVector{T},
    g_mat::SMatrix{2,2,T},
    endian_target::Int,
) where {T}
...
function apply_observable_kernel!(
    sv::CuDeviceVector{T},
    g_mat::SMatrix{N,N,T},
    flip_mask::CuDeviceVector{Int},
    ordered_ts::NTuple{Nq,Int},
) where {N,Nq,T}
    ix = (threadIdx().x - 1) + (blockIdx().x - 1) * blockDim().x
...
function apply_gate_single_ex_kernel!(
    cosϕ::T,
    sinϕ::T,
    state_vec::CuDeviceVector{T},
    endian_t1::Int64,
    endian_t2::Int64,
) where {T<:Complex}
    ix = (threadIdx().x - 1) + (blockIdx().x - 1) * blockDim().x
...
function apply_gate_double_ex_kernel!(
    cosϕ::T,
    sinϕ::T,
    state_vec::CuDeviceVector{T},
    endian_ts::NTuple{4,Int},
    ordered_ts::NTuple{4,Int},
) where {T<:Complex}
    ix = (threadIdx().x - 1) + (blockIdx().x - 1) * blockDim().x
...
function apply_gate_single_target_kernel!(
    g_00::T,
    g_01::T,
    g_10::T,
    g_11::T,
...
function apply_controlled_gate_kernel!(
    ::Val{1},
    g_00::T,
    g_01::T,
    g_10::T,
