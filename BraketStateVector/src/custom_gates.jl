...
apply_gate!(::Val{V}, g::MultiRZ, state_vec::StateVector{T}, t::Int) where {V,T<:Complex} =
...
apply_gate!(
    ...
    t1::Int,
    t2::Int,
) where {V,T<:Complex} = apply_gate!(Val(V), ZZ(g.angle), state_vec, t1, t2)
function apply_gate!(
    ::Val{V},
    g::MultiRZ,
    state_vec::StateVector{T},
    ...
    g_mat = SVector{2^N,ComplexF64}(exp(factor * r_mat[i]) for i = 1:2^N)
