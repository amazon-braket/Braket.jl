...
@testset "Adjoint Gradient" begin
    @testset "with nested targets" begin
        ...
        obs =
            (-2 * Braket.Observables.Y()) * (3 * Braket.Observables.I()) +
            0.75 * Braket.Observables.Y() * Braket.Observables.Z()
        ...
        @test res.values == [
            Dict(
                :expectation => 1.1920159847703675,
                :gradient => Dict(:theta => 5.880399467047451),
            ),
        ]
    ...
    @testset "with standard observable terms" begin
        ...
        inputs = Dict("theta" => 0.2)
        obs =
            2 * Braket.Observables.X() + 3 * Braket.Observables.Y() - Braket.Observables.Z()
    ...
    @testset "with batch" begin
        ...
        expected_result_values = [
            ...
            [
                Dict(
                    :expectation => 1.0430139866740715,
                    :gradient => Dict(:theta => 5.145349533666519),
                ),
            ],
        ]
