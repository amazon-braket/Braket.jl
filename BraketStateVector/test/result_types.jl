...
@testset "Result types" begin
    ...
    targs = collect(0:3)
    @testset "Simulation type $sim_type, f $f" for sim_type in (Val(:sv), Val(:dm)), f in funcs
        if sim_type isa Val{:sv}
            @testset "Amplitude" begin
                result_type = Braket.Amplitude(["0010", "0101", "1110"])
                amplitudes  = BraketStateVector.calculate(result_type, simulation(observable, f, sim_type))
                sv = state_vector()
        ...
        @testset "Probability" begin
            probability_12 = BraketStateVector.calculate(Braket.Probability([1, 2]), simulation(nothing, f, sim_type)) 
	    @test collect(probability_12) ≈ marginal_12()

	    state_vector_probabilities = collect(abs2.(state_vector()))
	    probability_all_qubits = collect(BraketStateVector.calculate(Braket.Probability([0, 1, 2, 3]), simulation(nothing, f, sim_type)))
        ...
        @testset "Expectation obs $obs" for obs in observables_testdata
            ...
            from_diagonalization = _expectation_from_diagonalization(BraketStateVector.state_with_observables(simulation(()->obs, f, sim_type)), obs[2], eigvals(obs[1]))
        ...
        @testset "Expectation no targets $obs" for obs in all_qubit_observables_testdata
            ...
            calculated           = [BraketStateVector.calculate(result_type, simulation(()->(obs, targs), f, sim_type)) for result_type in result_types]
            from_diagonalization = [_expectation_from_diagonalization(BraketStateVector.state_with_observables(simulation(()->(obs, t), f, sim_type)), t, eigvals(obs)) for t in targs]
            @test calculated     ≈ from_diagonalization
        end
        @testset "Variance obs $obs" for obs in observables_testdata
            ...
            @test result_type.targets == QubitSet(obs[2])
            calculated           = BraketStateVector.calculate(result_type, sim)
            from_diagonalization = _variance_from_diagonalization(BraketStateVector.state_with_observables(simulation(()->obs, f, sim_type)), obs[2], eigvals(obs[1]))
        ...
        @testset "Variance no targets $obs" for obs in all_qubit_observables_testdata 
            ...
            calculated           = [BraketStateVector.calculate(result_type, simulation(()->(obs, targs), f, sim_type)) for result_type in result_types]
            from_diagonalization = [_variance_from_diagonalization(BraketStateVector.state_with_observables(simulation(()->(obs, t), f, sim_type)), t, eigvals(obs)) for t in targs]
            @test calculated     ≈ from_diagonalization atol=1e-12
        end
